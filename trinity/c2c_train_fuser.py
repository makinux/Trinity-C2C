"""
P1: cache fuser の学習雛形
==========================
恒等射影＋手動ゲート(numpy) → 学習可能な torch nn.Module へ:
  - 層ごと head_dim 射影 W_k/W_v（学習）
  - 層ごとゲート（Gumbel-sigmoid@train / sigmoid@infer）
学習(C2C流): 送信/受信モデルは凍結。受信モデルに『fused KV(微分可能)』を注入し、
  ターゲット継続の LM 交差エントロピーを最小化 → fuserのみ更新。
  （凍結LMの重みは grad 不要だが、注入KVに対する forward は微分可能なので fuser へ勾配が届く）

検証:
  python -m trinity.c2c_train_fuser --selftest   # torchのみ。autograd/最適化/ゲート学習を検証（モデル不要）
  python -m trinity.c2c_train_fuser --real        # Qwen2.5-0.5B(CPU)で「凍結LM越しに勾配が届く」ことを実証
"""
import sys
import math
import torch
...
class TorchC2CFuser(nn.Module):
    def __init__(self, sharer: KVShape, receiver: KVShape, init_gate: float = 0.05, tau: float = 1.0):
        ...
        if sharer.n_heads != receiver.n_heads:
            raise ValueError("GQAヘッド数不一致は射影/対応付けが必要（同一ファミリ前提）")
    ...
    def gates(self) -> torch.Tensor:
        if self.training:                                  # Gumbel-sigmoid（reparam・微分可能）
    ...
    def forward(self, recv_layers, share_layers):
        """recv/share: list[(K,V)], K/V=[heads,seq,hd]（凍結基盤からの定数）→ fused list（微分可能）。"""
        g = self.gates()
        out = []
        for i, (Kr, Vr) in enumerate(recv_layers):
            ...
            gi = g[i]
            Kf = torch.cat([(1 - gi) * Kr[:, :L] + gi * Ksp[:, :L], Kr[:, L:]], dim=1)   # in-place回避
...
def _encode_layers(model, tok, text):
    ...
    with torch.no_grad():                                  # 基盤は凍結＝定数エンコード
...
def _cache_from(fused, upto):
    ...
    for i, (K, V) in enumerate(fused):
        c.update(K[:, :upto, :].unsqueeze(0), V[:, :upto, :].unsqueeze(0), i)   # grad付きテンソルを投入
    return c


def neg_logp_target(model, tok, fused, recv_ids, target: str):
    """fused(受信文脈KV) の末尾を落とし、受信末尾トークンをライブ入力 → target先頭トークンの -logp（微分可能）。"""
    L = fused[0][0].shape[1]
...
def train_fuser(model, tok, fuser, triples, steps=30, lr=0.05, log=True):
    """triples: [(share_ctx, recv_ctx, target)]。fuser のみ更新。"""
    ...
    for step in range(steps):
        ...
        loss.backward()
        if step == 0:                                       # 核心: 凍結LM越しに勾配が届いているか
            g = fuser.gate_logit.grad
            assert g is not None and torch.isfinite(g).all(), "勾配がfuserへ届いていない（注入経路が断絶）"
            assert fuser.Wk["0"].grad is not None, "射影に勾配が届いていない"
...
# ============================================================
# selftest（torchのみ・モデル不要）: autograd/最適化/ゲート学習
# ============================================================
def selftest():
    torch.manual_seed(0)
    ...
    losses = []
    for _ in range(200):
        fused = fuser(recv, share)                          # 目標: fused が share に一致（要 gate→1, W→I）
    ...
    print(f"[selftest] fuser trains : loss {losses[0]:.3f}->{losses[-1]:.3f} / gate {g0:.2f}->{g1:.2f} / grad→W ✓")
    print("[selftest] ALL PASSED")


# ============================================================
# real smoke（Qwen2.5-0.5B・CPU）: 凍結LM越しに勾配がfuserへ届くか
# ============================================================
def real_smoke(model_name="Qwen/Qwen2.5-0.5B-Instruct"):
    ...
    print("[real] 目標: Japan文脈 に France KV を融合し ' Paris' を予測するよう fuser を学習")
    train_fuser(model, tok, fuser, triples, steps=12, lr=0.1)

    # アブレーション（Codex推奨）: 注入の効果と「share内容を実際に使っているか/記憶か」を切り分け
    fuser.eval()
    ...
    with torch.no_grad():
        ...
        l_gate0 = neg_logp_target(model, tok, rc_l, rc_ids, " Paris").item()              # 注入なし
        l_shuf = neg_logp_target(model, tok, fuser(rc_l, sh_G), rc_ids, " Paris").item()  # 誤share
    print(f"[ablation] -logp(Paris): learned(France)={l_learn:.2f}  gate0/no-inj={l_gate0:.2f}  shuffled(Germany)={l_shuf:.2f}")
    print("  learned≪gate0 → 注入が効く。 learned≪shuffled → share内容を実際に使用 / 近ければ記憶(=データ拡張が必要)。")
    print("→ 勾配アサーション通過＝『凍結LM越しに勾配が届き、fuserが学習可能』を実証。")
