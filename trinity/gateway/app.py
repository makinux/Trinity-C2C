...
def create_app() -> FastAPI:
    ...
    async def v1_chat_completions(req: ChatCompletionRequest):
        ...
        mock = service.resolve_mock(req.trinity_mock)
        if req.stream:
            gen = service.stream_openai(query, mock=mock, max_turns=req.trinity_max_turns)
            return StreamingResponse(gen, media_type="text/event-stream",
                                     headers=service.SSE_HEADERS)
        result, trace = await service.run_collect(query, mock=mock,
    ...
    async def debug_runs_stream(req: DebugRunRequest):
        gen = service.stream_debug_sse(
            req.query, mock=req.mock, max_turns=req.max_turns,
